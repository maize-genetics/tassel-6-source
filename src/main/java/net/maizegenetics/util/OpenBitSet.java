...
public class OpenBitSet implements BitSet, Cloneable, Serializable {
    ...
    public OpenBitSet(long[] bits, int numWords) {
        if (numWords > bits.length) {
            throw new IllegalArgumentException("OpenBitSet: init: num of words should be less than or equal to bits length.");
        }
        myBits = bits;
        myNumWords = numWords;
    }
